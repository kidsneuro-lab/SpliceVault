...
server <- function(input, output, session) {
  ...
  {
    flog.debug("List of transcripts based on selected gene")
    ...
    switch (input$dbInput,
            ...
            "300K-RNA (hg38)" = {
              ...
              if (input$txInput != "" & input$txInput %in% txs$transcript_id) {
  ...
  observeEvent({
    ...
    flog.debug("DB %s", isolate(input$dbInput))
    flog.debug("Transcript %s", isolate(input$txInput))
    flog.debug("Splice site type %s", isolate(input$ssTypeInput))
    flog.debug("Transcript type %s", isolate(input$txTypeInput))
    ...
    switch (input$dbInput,
            ...
            "300K-RNA (hg38)" = {
              ...
              if (input$exonInput %in% ex$exon_id) {
                presel = ex$exon_id
              } else {
                presel = ex$exon_id[1]
              }
              ...
              updateSelectizeInput(
                session = session,
                inputId = 'exonInput',
                choices = setNames(ex$exon_id, ex$display_value),
                selected = presel,
                server = TRUE
              )
            })
